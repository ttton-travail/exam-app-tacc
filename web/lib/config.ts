...
// -----------------------------------------------
// 選択肢の再生成（不備のある問題の作り直し）専用設定
// -----------------------------------------------
// 通常生成は Lite だが、税額・控除額などの計算問題は Lite の能力を超えやすく、作り直しても
// 同じ Lite ではまた 0埋め・重複を返して直らない。そこで「再生成パスだけ」上位モデル（Flash）＋
// 低温で確実に解かせる。再生成は不備のあった数問だけ・1リクエストなので、上位モデルでもコスト増はわずか。
export const GEMINI_REPAIR_MODEL = 'gemini-2.5-flash'
// 作り直しは正確さが要る作業（多様性は不要）。温度は低めにして、計算結果のブレと
// 「解けず0へ逃げる」挙動を抑える。一括生成の温度（多様性確保）はそのまま据え置く。
export const GEMINI_REPAIR_TEMPERATURE = 0.4
// 不備が無くなるまで再生成を繰り返す最大回数（無限ループ・コスト暴走の歯止め）。
// 上限まで直らなかった問題は、ユーザーに出さず表示から除外する。
export const MAX_CHOICE_REPAIR_ROUNDS = 3
